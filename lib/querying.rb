def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT Books.title, books.year FROM Books JOIN series ON Series.id = series_id WHERE series_id = 1 ORDER by year "
end
...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name AS AuthorsName,subgenres.name FROM SubGenres JOIN Authors ON authors.id = subgenres.id;"
end
